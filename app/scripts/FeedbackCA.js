	...
	if(KCcheat == 'true')
		{
			...
			for (i = 0; i < form.Quiz.length; i++) 
				{
					if(vCorrectAnswer.charAt(i)==1) 
						{	form.elements[i].style.backgroundColor = '#F8F7C7';}						
				}
		}
}
...
{	
	...
	for (var i=0; i < form.Quiz.length; i++)  
		{
			if(form.Quiz[i].checked) {vAnswered = 1}
			vResponse = vResponse + form.Quiz[i].checked*1	
		}
	if (vAnswered == 0)		{alert("\nYou need to answer the question to continue.")}
						...
						//alert(vResponse);
							for (i = 0; i < form.Quiz.length; i++) 
							{
								if (vResponse.charAt(i) == vCorrectAnswer.charAt(i))
									...
									if(form.Quiz[i].checked==1){vCorrectCount = vCorrectCount + 1}
									}
								 else
									{	
									//uncheck the incorrect ones
									form.Quiz[i].checked=0
									}
							
							}
							//tell them to try again and how many they got right
							if(vCorrectCount == 0)
								{DisplayFeedback('Incorrect','You did not answer any item correctly.  <strong>Please review the question and begin again.</strong> ')}
							else
								{DisplayFeedback('Incorrect','You answered ' + vCorrectCount + '</strong> of the items correctly.  Your wrong answers will be erased to allow you to try again.</p> <p><strong>Please select Try Again and then select a new response for each "unanswered" item.  Remember, the remaining selected answers are correct! </strong>')}
				  ...
				  if (vResponse != vCorrectAnswer)		
				  	{
						...
						//tell them to try again and how many they got right
					if(vCorrectCount == 0)
						{DisplayFeedback('Incorrect','You did not answer any item correctly.   </p><p>' + Feedback)}
					else
